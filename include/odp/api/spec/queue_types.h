...
typedef struct odp_queue_param_t {
	...
	/** Vector configuration parameters
	  ...
	  * using vector aggregator 0 by default. odp_queue_enq_vector()
	  * function can be used to utilize other vector aggregators.
	  *
	  * Implementation won't split up event vectors successfully enqueued
	  * by the application.
	  *
	  * Application is not allowed to enqueue event vector events
	  * (ODP_EVENT_VECTOR) to a queue with vector aggregation enabled (i.e.
