 ...
 *
 *  A LSO operation may perform segmentation on these protocols.
 */
...
typedef struct odp_lso_capability_t {
	...
	/** Maximum number of segments a LSO operation may create. This implies that
	 *  the maximum supported input packet payload size for a LSO operation is
	 *  max_segments * max_payload_len bytes. */
	uint32_t max_segments;

	/** Maximum payload length per a LSO generated packet (in bytes). This is the maximum value
 ...
 * automatically modified before transmission. Header updates are based on the defined L3 and L4
 * protocols. Header checksums are updated after modifications. L3/L4 protocol modifications
 * require that L3/L4 layer offsets in the packet are valid (see e.g. odp_packet_l3_offset()).
 * Segmentation is done on the protocol specified by the profile (see e.g. ODP_LSO_PROTO_TCP_IPV4,
 * ODP_LSO_PROTO_IPV4, etc). In addition, custom field updates may be used to cover unsupported
 * or proprietary protocols. Custom fields must not overlap with each other and can be used only
 * when ODP_LSO_PROTO_CUSTOM is selected.
 *
 * Packet are processed and transmitted in the array order. Segments of each packet are transmitted
 * in ascending order.
 *
