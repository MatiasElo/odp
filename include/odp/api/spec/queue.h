 ...
 *
 * Vector aggregator index must be smaller than the configured number of
 * vector aggregators (odp_queue_param_t::num_vector). Event ordering is not
 * maintained across multiple vector aggregators within a queue.
 *
 * ToDo:
 * - OK to enqueue event_vectors?
 * - odp_queue_enq_vector_multi()
 *
 * @param queue        Queue handle
 * @param vector_index Vector aggregator index
 * @param ev           Event handle
 ...
 * @retval <0 on failure
 */
int odp_queue_enq_vector(odp_queue_t queue, uint32_t aggregator_index, odp_event_t ev,
