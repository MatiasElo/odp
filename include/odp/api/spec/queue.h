 ...
 * generation.
 *
 * Depending on the implementation there may be some delay in updating the queue
 * length value. If reading queue length is not supported (see queue-type-
 * specific capabilities), zero is always returned.
 *
 * @param queue   Queue handle
 *
