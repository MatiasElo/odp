 ...
 *
 * Like odp_queue_enq(), but enqueues event to queue's vector event aggregator.
 * Vector aggregator index must be smaller than the configured number of vector
 * aggregators for the queue (odp_queue_param_t::num_vector). Aggregators are
 * indexed in the same order as they were given in odp_queue_param_t::vector
 * array.
 *
