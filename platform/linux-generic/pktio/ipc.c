...
/* create the ring */
static ring_mpmc_rst_ptr_t *_ring_create(const char *name, uint32_t count,
				uint32_t shm_flags)
{
				   ...
				   odp_packet_t pkt_table[], int len)
{
	...
	if (pkts == 0) {
		...
		r = pktio_ipc->rx.recv;
		pkts = ring_mpmc_rst_ptr_deq_multi(r, ring_mask, ipcbufs_p,
					  len);
		if (odp_unlikely(pkts < 0))
			_ODP_ABORT("internal error dequeue\n");
	}
	...
	if (pkts != i) {
		...
		ring_mpmc_rst_ptr_enq_multi(r_p, ring_mask, ipcbufs_p,
				   pkts - i);
