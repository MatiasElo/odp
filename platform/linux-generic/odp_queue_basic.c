...
static int plain_queue_enq(odp_queue_t handle, _odp_event_hdr_t *event_hdr)
{
	...
	num_enq = ring_mpmc_ptr_enq(ring_mpmc, queue->ring_data, queue->ring_mask,
				    (uintptr_t)event_hdr);
...
static inline int plain_queue_enq_multi(odp_queue_t handle, _odp_event_hdr_t *event_hdr[], int num)
{
	...
	num_enq = ring_mpmc_ptr_enq_multi(ring_mpmc, queue->ring_data, queue->ring_mask,
...
static _odp_event_hdr_t *plain_queue_deq(odp_queue_t handle)
{
	...
	num_deq = ring_mpmc_ptr_deq(ring_mpmc, queue->ring_data, queue->ring_mask,
				    (uintptr_t *)&event_hdr);
	if (num_deq == 0)
		return NULL;
...
static int sched_queue_enq(odp_queue_t handle, _odp_event_hdr_t *event_hdr)
{
	...
	num_enq = ring_st_ptr_enq(ring_st, queue->ring_data, queue->ring_mask,
				  (uintptr_t)event_hdr);
