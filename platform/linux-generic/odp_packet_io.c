...
{
	lso_profile_t *lso_prof = (lso_profile_t *)(uintptr_t)lso_profile;
...
int odp_packet_lso_request(odp_packet_t pkt, const odp_packet_lso_opt_t *lso_opt)
{
	...
	lso_profile_t *lso_prof = (lso_profile_t *)(uintptr_t)lso_opt->lso_profile;

	if (lso_opt->lso_profile == ODP_LSO_PROFILE_INVALID || lso_prof->used == 0) {
	...
	odp_packet_payload_offset_set(pkt, lso_opt->payload_offset);
...
static int lso_update_custom(lso_profile_t *lso_prof, odp_packet_t pkt, int segnum)
{
	...
	for (i = 0; i < num_custom; i++) {
		...
		if (size == 8)
			u64 = odp_be_to_cpu_64(u64);
		else if (size == 4)
			u32 = odp_be_to_cpu_32(u32);
		else if (size == 2)
			u16 = odp_be_to_cpu_16(u16);
			   ...
			   const odp_packet_lso_opt_t *lso_opt)
{
	...
	if (hdr_len > pkt_len || payload_len > pkt_payload) {
		ODP_ERR("Too short packet\n");
		return -1;
	}
			...
			const odp_packet_lso_opt_t *opt)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (pktout_send_lso(queue, pkt, opt_ptr)) {
