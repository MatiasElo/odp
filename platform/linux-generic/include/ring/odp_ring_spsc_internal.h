...
#undef _RING_SPSC_DEQ_BATCH
#undef _RING_SPSC_ENQ
#undef _RING_SPSC_ENQ_MULTI
#undef _RING_SPSC_ENQ_BATCH
...
#error Include type specific ring header instead of this common file.
	...
	#define _RING_SPSC_DEQ_BATCH	ring_spsc_u32_deq_batch
	#define _RING_SPSC_ENQ		ring_spsc_u32_enq
	#define _RING_SPSC_ENQ_MULTI	ring_spsc_u32_enq_multi
	#define _RING_SPSC_ENQ_BATCH	ring_spsc_u32_enq_batch
	...
	#define _ring_spsc_gen_t	ring_spsc_u64_t
	#define _ring_spsc_data_t	uint64_t

	#define _RING_SPSC_INIT		ring_spsc_u64_init
	#define _RING_SPSC_DEQ		ring_spsc_u64_deq
	#define _RING_SPSC_DEQ_MULTI	ring_spsc_u64_deq_multi
	#define _RING_SPSC_DEQ_BATCH	ring_spsc_u64_deq_batch
	#define _RING_SPSC_ENQ		ring_spsc_u64_enq
	#define _RING_SPSC_ENQ_MULTI	ring_spsc_u64_enq_multi
	#define _RING_SPSC_ENQ_BATCH	ring_spsc_u64_enq_batch
	#define _RING_SPSC_IS_EMPTY	ring_spsc_u64_is_empty
	#define _RING_SPSC_LEN		ring_spsc_u64_len
	...
	#define _RING_SPSC_DEQ_BATCH	ring_spsc_ptr_deq_batch
	#define _RING_SPSC_ENQ		ring_spsc_ptr_enq
	#define _RING_SPSC_ENQ_MULTI	ring_spsc_ptr_enq_multi
	#define _RING_SPSC_ENQ_BATCH	ring_spsc_ptr_enq_batch
...
/* Dequeue a batch of data from the ring head. Returns a full batch or nothing. */
static inline uint32_t _RING_SPSC_DEQ_BATCH(_ring_spsc_gen_t *ring,
					    _ring_spsc_data_t *ring_data,
					    uint32_t ring_mask, _ring_spsc_data_t data[],
					    uint32_t max_num)
{
	...
	odp_atomic_store_rel_u32(&ring->r.head, head + max_num);
...
/* Enqueue a batch of data into the ring tail. Stores a full batch or nothing. */
static inline uint32_t _RING_SPSC_ENQ_BATCH(_ring_spsc_gen_t *ring,
					    _ring_spsc_data_t *ring_data,
					    uint32_t ring_mask,
					    const _ring_spsc_data_t data[],
					    uint32_t num_data)
{
	...
	odp_atomic_store_rel_u32(&ring->r.tail, tail + num_data);
